...
from utils.state import ObservableState, FullState, SmallState
...
class Agent(object):
    ...
    def get_small_state(self):
        return SmallState(self.vx, self.vy, self.radius)
    ...
    def step(self, action):
        ...
        """
        self.check_validity(action)
        self.px, self.py = self.compute_position(action, self.time_step)
        #print(action, self.px, self.py)
        if self.kinematics == 'holonomic':
            self.vx = action.vx
            self.vy = action.vy
        else:
